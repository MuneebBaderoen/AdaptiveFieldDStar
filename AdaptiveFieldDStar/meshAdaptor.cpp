...
bool MeshAdaptor::triangle_is_sliver(const Point * pts, Facet_circulator hfc){
	using namespace std;
	...
	if(longestEdge/shortestEdge>=2){
		...
		bisect_sliver(longestIndex, mid, h);
		return true;
	}
...
void MeshAdaptor::bisect_sliver(int halfedgeIndex, Point midEdge, Halfedge_handle h){
...
void MeshAdaptor::split_cell(Facet_circulator hfc){
	using namespace std;
	...
	if(!triangle_is_sliver(points, hfc)){
